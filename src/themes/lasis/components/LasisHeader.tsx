...
import { LasisBookingModal } from './LasisBookingModal';
    ...
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-700 ease-in-out ${bgColor}`}>
      <div className="max-w-7xl mx-auto px-6 lg:px-8 py-6 flex items-center justify-between">
        ...
        <nav className={`hidden md:flex gap-10 items-center justify-start flex-1 font-serif text-sm italic ${textColor} transition-colors duration-500`}>
          <Link href="/lasis-klinik/hizmetler" className="hover:text-[#D4A373] transition-colors">Hizmetlerimiz</Link>
          <Link href="/lasis-klinik/hikayemiz" className="hover:text-[#D4A373] transition-colors">Hikayemiz</Link>
        </nav>

        {/* Brand Logo */}
        <Link href="/lasis-klinik" className={`absolute left-1/2 -translate-x-1/2 flex flex-col items-center justify-center z-[60] ${textColor} transition-colors duration-500`}>
          <span className="text-4xl md:text-5xl font-serif tracking-wide">Lasis Klinik</span>
          <span className="text-[9px] font-sans tracking-[0.3em] uppercase opacity-70 mt-1">Medikal Estetik</span>
        </Link>
        ...
        <div className="flex-1 flex items-center justify-end gap-4 md:gap-6 z-[60]">
          ...
          <nav className={`hidden md:flex gap-10 items-center font-serif text-sm italic ${textColor} transition-colors duration-500`}>
            <Link href="/lasis-klinik/galeri" className="hover:text-[#D4A373] transition-colors">Galeri</Link>
            <Link href="/lasis-klinik/iletisim" className="hover:text-[#D4A373] transition-colors">İletişim</Link>
          </nav>
          ...
          <button onClick={() => window.dispatchEvent(new Event('openLasisBooking'))} className="hidden md:block bg-[#D4A373] text-[#FDFCF9] font-sans font-medium text-xs tracking-widest uppercase px-6 py-3 rounded-full hover:bg-[#3E2B2B] transition-colors duration-500 shadow-md not-italic">
            Randevu Al
          </button>
        </div>
        ...
        {isMobileMenuOpen && (
          ...
          >
            <nav className="flex flex-col gap-6 text-center text-[#3E2B2B] font-serif text-xl italic">
              <Link href="/lasis-klinik/hizmetler" onClick={() => setIsMobileMenuOpen(false)}>Hizmetlerimiz</Link>
              <Link href="/lasis-klinik/hikayemiz" onClick={() => setIsMobileMenuOpen(false)}>Hikayemiz</Link>
              <Link href="/lasis-klinik/galeri" onClick={() => setIsMobileMenuOpen(false)}>Galeri</Link>
              <Link href="/lasis-klinik/iletisim" onClick={() => setIsMobileMenuOpen(false)}>İletişim</Link>
              <button 
                onClick={() => {
                  setIsMobileMenuOpen(false);
                  window.dispatchEvent(new Event('openLasisBooking'));
                }}
